...
(function () {
  ...
  const BLUE_STRONG = '#1f3a8a';
  const BLUE_MID = '#3b83f6';
  const BLUE_LIGHT = '#94c5fd';
  const NO_COOP = '#dde4ed';
  const MAP_BG = '#cee5f5';
  const ORANGE_STRONG = '#b45309';
  const ORANGE_MID = '#d97706';
  ...
  const TIERS = [
    { min: 5000, fill: BLUE_STRONG, stroke: BLUE_STRONG, label: '深度合作 ≥5,000篇'   },
    { min: 500,  fill: BLUE_MID, stroke: BLUE_MID, label: '中度合作 500–4,999篇' },
    { min: 1,    fill: BLUE_LIGHT, stroke: BLUE_MID, label: '少量合作 <500篇'       },
    { min: 0,    fill: NO_COOP, stroke: '#c5cdd9', label: '无合作记录'            }
  ];
  ...
  let resizeTimer;
  let ov2ZoomTransform = d3.zoomIdentity;
  ...
  function initTooltip() {
    ...
    window.ov2ShowTip = function(e, d) {
      ...
      const growthNum = growth === '—' ? null : +growth;
      const growthColor = growthNum == null ? '#94a3b8' : growthNum >= 0 ? POSITIVE : NEGATIVE;
      const growthRow = p === 'p16' ? `
        <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
          ...
          <span style="color:${growthColor};">${growth === '—' ? '—' : `${growthNum >= 0 ? '+' : ''}${growth}%`}</span>
        </div>
      ` : '';
      ...
      tt.innerHTML = `
        ...
        <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
          ...
          <span style="font-weight:600;color:${BLUE_LIGHT};">${window.fmt(d[p])} 篇</span>
        </div>
        ${growthRow}
        <div style="display:flex;justify-content:space-between;gap:12px;">
          <span style="color:rgba(255,255,255,.5);">全球排名</span>
          <span style="color:${ORANGE_MID};font-weight:700;">#${rank}</span>
        </div>
  ...
  function injectHTML() {
    ...
    el.innerHTML = `
      <div style="margin-bottom:18px;">
        <h3 style="margin:0 0 6px;font-size:1.5rem;font-weight:700;line-height:1.25;color:#0f172a;">
          一、全球合作概览
        </h3>
        <p style="margin:0;color:#6b7280;font-size:1rem;line-height:1.7;">
          展示中国在全球科研合作中的整体分布，为深入观察中东欧合作建立背景和对比基准。
        </p>
      </div>
      ...
      <div id="ov2-wrap" style="display:grid;grid-template-columns:minmax(0,0.72fr) minmax(260px,0.28fr);gap:18px;align-items:stretch;">
        <div style="background:#fff;border-radius:14px;padding:18px 20px 14px;
          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;
            ...
            <div>
              ...
              <div style="font-size:11.5px;color:#64748b;">
                按合作发文量三档分类着色 ·
                <span style="color:${ORANGE_MID};font-weight:600;">橙框</span> = 中东欧16国 · 滚轮缩放 · 拖拽平移
              </div>
            </div>
            <div style="display:inline-flex;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
              <button id="ov2-p11" style="padding:5px 14px;font-size:12px;font-weight:600;
                border:none;background:${BLUE_STRONG};color:#fff;cursor:pointer;">2011–2015</button>
          ...
          <div id="ov2-mapwrap" style="position:relative;border-radius:10px;overflow:hidden;
            background:${MAP_BG};min-height:400px;flex:1;">
        ...
        <div id="ov2-sidepanel" style="display:flex;flex-direction:column;gap:14px;position:sticky;top:12px;">
          <div style="background:#fff;border-radius:14px;padding:16px 18px 14px;
            box-shadow:0 2px 10px rgba(0,0,0,.07);border:1px solid #dde4ec;">
            <div style="font-size:15px;font-weight:700;color:#0f172a;margin-bottom:10px;">数据说明</div>
            <div id="ov2-copy-data" style="font-size:13px;line-height:1.7;color:#334155;"></div>
            <div id="ov2-copy-stats" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:14px;"></div>
          </div>
          <div style="background:#fff;border-radius:14px;padding:16px 18px 16px;
            box-shadow:0 2px 10px rgba(0,0,0,.07);border:1px solid #dde4ec;">
            <div style="font-size:15px;font-weight:700;color:#0f172a;margin-bottom:10px;">数据洞察</div>
            <div id="ov2-copy-insights" style="font-size:13px;line-height:1.65;color:#334155;"></div>
          </div>
        </div>
      </div>
      ...
      <div style="margin:26px 0 18px;">
        ...
        <p style="margin:0;color:#6b7280;font-size:1rem;line-height:1.7;">
          呈现2011–2020年中国与中东欧16国的合作总量、年度占比及增长趋势，判断合作是否在持续扩展和深化。
        </p>
      </div>
  ...
  function renderNarrative() {
    const dataEl = document.getElementById('ov2-copy-data');
    const statsEl = document.getElementById('ov2-copy-stats');
    const insightsEl = document.getElementById('ov2-copy-insights');
    if (!dataEl || !statsEl || !insightsEl) return;

    const period = currentPeriod;
    const periodLabel = period === 'p16' ? '2016–2020' : '2011–2015';
    const prevLabel = period === 'p16' ? '2011–2015' : '2016–2020';
    ...
    const ceeP11 = d3.sum(window.CEE_COUNTRIES, d => d.p11 || 0);
    const ceeP16 = d3.sum(window.CEE_COUNTRIES, d => d.p16 || 0);
    const ceeGrowth = ceeP11 > 0
      ? ((ceeP16 - ceeP11) / ceeP11 * 100).toFixed(1)
      : '—';
    const top5 = window.RAW_DATA
      .filter(d => (d[period] || 0) > 0)
      .sort((a, b) => b[period] - a[period])
      .slice(0, 5);
    const top3 = top5.slice(0, 3);
    const topCee = [...window.CEE_COUNTRIES]
      .sort((a, b) => (b[period] || 0) - (a[period] || 0))
      .slice(0, 3);

    dataEl.innerHTML = `
      <div style="color:#475569;">
        这张图展示了中国科研合作网络的全球分布，颜色越深表示合作发文量越高。从整体上看，合作高度集中在美国、英国、澳大利亚、德国、日本等传统科研合作中心。
      </div>`;

    const statCards = [
      { label: '当前时期', value: periodLabel, color: '#0f172a' },
      { label: '全球合作总量', value: `${window.fmt(total)} 篇`, color: BLUE_STRONG },
      { label: '中东欧合作总量', value: `${window.fmt(ceeTotal)} 篇`, color: ORANGE_MID },
      { label: '中东欧占比', value: `${share}%`, color: ORANGE_MID }
    ];
    statsEl.innerHTML = statCards.map(card => `
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px 12px;">
        <div style="font-size:11px;color:#64748b;margin-bottom:6px;">${card.label}</div>
        <div style="font-size:15px;font-weight:700;color:${card.color};line-height:1.35;">${card.value}</div>
      </div>
    `).join('');

    const insightItem = (title, text) => `
      <div style="margin-bottom:12px;">
        <div style="font-size:12px;font-weight:700;color:${BLUE_STRONG};margin-bottom:4px;">${title}</div>
        <div style="color:#334155;font-size:12.5px;line-height:1.65;">${text}</div>
      </div>`;

    const top3Detail = top3.map((d, i) => `<strong>${i + 1}. ${d.cn}</strong>（${window.fmt(d[period])}篇）`).join('、');
    const ceeLeaders = topCee.map(d => `<strong style="color:${ORANGE_MID};">${d.cn}</strong>`).join('、');
    const globalShare = `<span style="color:${BLUE_STRONG};font-weight:700;">${share}%</span>`;
    const ceeTotalText = `<span style="color:${ORANGE_MID};font-weight:700;">${window.fmt(ceeTotal)} 篇</span>`;
    const globalTotalGrowth = `<span style="color:${BLUE_STRONG};font-weight:700;">${((911293 - 427285) / 427285 * 100).toFixed(1)}%</span>`;
    const ceeGrowthText = `<span style="color:${ORANGE_MID};font-weight:700;">${ceeGrowth}%</span>`;

    let insights;
    if (period === 'p11') {
      insights = [
        ['全球格局', `合作高地主要集中在<strong>美国、英国、澳大利亚、德国、日本</strong>等传统科研合作中心；前列国家以 ${top3Detail} 为主。`],
        ['中东欧位置', `中东欧16国合作总量为 ${ceeTotalText}，占全球合作总量的 ${globalShare}，整体规模相对有限，但已形成稳定参与；区域内以 ${ceeLeaders} 等国为主。`],
        ['变化趋势', `进入 2016–2020 后，中东欧合作总量较本阶段增长约 ${ceeGrowthText}，显示该区域合作热度正在提升。`]
      ];
    } else {
      insights = [
        ['全球格局', `合作仍高度集中于<strong>美国、英国、澳大利亚、加拿大、德国</strong>等传统科研合作中心；前三位为 ${top3Detail}，全球合作总量较上一阶段增长约 ${globalTotalGrowth}。`],
        ['中东欧位置', `中东欧16国合作总量为 ${ceeTotalText}，占全球 ${globalShare}，规模仍低于美英等核心国家，但在全球网络中保持可见存在感；区域内以 ${ceeLeaders} 等国为主。`],
        ['变化趋势', `相比 ${prevLabel}，中东欧合作总量变化约 ${ceeGrowthText}，增速高于全球平均水平，是值得持续观察的合作板块。`]
      ];
    }

    insightsEl.innerHTML = insights.map(([t, txt]) => insightItem(t, txt)).join('');
  }
  ...
  async function drawMap() {
    ...
    const H = Math.max(Math.round(W * 0.53), 400);
    ...
    defs.append('clipPath')
      .attr('id', 'ov2-map-clip')
      .append('rect')
      .attr('width', W)
      .attr('height', H);

    const root = svg.append('g')
      .attr('class', 'ov2-map-root')
      .attr('clip-path', 'url(#ov2-map-clip)');

    root.append('rect').attr('width', W).attr('height', H).attr('fill', MAP_BG);
    root.append('path').datum(d3.geoGraticule().step([7.5, 7.5])())
      .attr('d', path).attr('fill', 'none')
      .attr('stroke', '#d3deea').attr('stroke-width', 0.3);
    ...
    const zoom = d3.zoom()
      ...
      .on('zoom', (event) => {
        ov2ZoomTransform = event.transform;
        root.attr('transform', ov2ZoomTransform);
      })
      .on('end', () => svg.style('cursor', 'grab'));

    const defaultScale = 1.0;
    const centeredTranslateX = (W * (1 - defaultScale)) / 2;
    const centeredTranslateY = (H * (1 - defaultScale)) / 2 - 5;
    const initialTransform =
      (ov2ZoomTransform && (ov2ZoomTransform.k !== 1 || ov2ZoomTransform.x !== 0 || ov2ZoomTransform.y !== 0))
        ? ov2ZoomTransform
        : d3.zoomIdentity.translate(centeredTranslateX, centeredTranslateY).scale(defaultScale);
    ov2ZoomTransform = initialTransform;
  ...
  function buildLayers(root, path) {
    ...
    root.selectAll('path.cp').data(worldGeoData.features).join('path')
      ...
      .attr('stroke', f => window.dataByNum[f.id]?.cee ? ORANGE_MID : 'rgba(255,255,255,.55)')
      ...
      .on('mouseout', function(e, f) {
        const d = window.dataByNum[f.id];
        d3.select(this)
          .attr('stroke', d?.cee ? ORANGE_MID : 'rgba(255,255,255,.55)')
  ...
  function renderLegend() {
    ...
    `).join('') + `
      <div style="display:flex;align-items:center;gap:5px;font-size:11px;color:#4b5563;">
        <div style="width:13px;height:9px;border:2px solid ${ORANGE_MID};border-radius:2px;"></div>
        中东欧16国边框标注
      </div>
    `;
  }
  ...
  function switchPeriod(p) {
    ...
    if (p === 'p16') {
      b16.style.background = BLUE_STRONG; b16.style.color = '#fff'; b16.style.fontWeight = '600';
      b11.style.background = '#f8fafc'; b11.style.color = '#64748b'; b11.style.fontWeight = '500';
    } else {
      b11.style.background = BLUE_STRONG; b11.style.color = '#fff'; b11.style.fontWeight = '600';
  ...
  function init() {
    ...
    function applyResp() {
      const s = document.getElementById('ov2-copy-stats');
      if (s) s.style.gridTemplateColumns = window.innerWidth < 560 ? '1fr' : 'repeat(2,minmax(0,1fr))';
    }
    applyResp();
...
(function () {
  ...
  // =========================================================
  const REGION_COLOR = {
    '北美': BLUE_STRONG, '西欧': BLUE_MID, '东亚': BLUE_MID, '东南亚': BLUE_LIGHT,
    '中东欧': ORANGE_MID, '中东': BLUE_LIGHT, '南亚': BLUE_MID, '东欧': BLUE_LIGHT,
    '拉丁美洲': BLUE_LIGHT, '非洲': BLUE_LIGHT, '大洋洲': BLUE_MID, '中亚': BLUE_LIGHT
  };
    ...
    window.rk2ShowTip = function(e, d, isRegion) {
      ...
      if (isRegion) {
        tt.innerHTML = `
          ...
          <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
            <span style="color:rgba(255,255,255,.5);">占全球比例</span><span style="color:${BLUE_LIGHT};font-weight:600;">${pctFmt(getSharePct(d[p], p))}%</span>
          </div>
        ...
        tt.innerHTML = `
          ...
          <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
            ...
            <span style="color:${BLUE_LIGHT};font-weight:600;">${pctFmt(regionShare)}%</span>
          </div>
          <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
            <span style="color:rgba(255,255,255,.5);">占全球比例</span>
            <span style="color:${BLUE_LIGHT};font-weight:600;">${pctFmt(getSharePct(d[p], p))}%</span>
          </div>
          ${p === 'p16' ? `<div style="display:flex;justify-content:space-between;gap:12px;">
            <span style="color:rgba(255,255,255,.5);">增幅</span>
            <span style="color:${growthColor};">${growth == null ? '—' : `+${pctFmt(growth)}%`}</span>
          </div>` : ''}
          ${d.cee?`<div style="margin-top:6px;background:${ORANGE_STRONG};border-radius:4px;padding:2px 7px;font-size:11px;font-weight:600;">★ 中东欧</div>`:''}`;
      }
      tt.style.display = 'block';
      window.rk2MoveTip(e);
    };
  ...
  function injectHTML() {
    ...
    el.innerHTML = `
      <div style="background:#fff;border-radius:14px;padding:20px 22px;
          ...
          <div>
            ...
            <div style="font-size:11.5px;color:#64748b;">
              左栏：按地区汇总 · 右栏：点击地区查看内部国家排名 ·
                <span style="color:${ORANGE_MID};font-weight:600;">橙色</span> = 中东欧国家
            </div>
          </div>
        ...
        <div id="rk2-callout" style="margin-top:18px;border-radius:10px;padding:16px 18px;
          background:#ffffff;
          border:1px solid #dbe5ef;
          box-shadow:0 10px 24px rgba(15,23,42,.04);"></div>
  ...
  function renderRegionBars() {
    ...
    regions.forEach((d, i) => {
      ...
      g.append('text')
        ...
        .attr('fill', isSel ? (isCEE ? ORANGE_STRONG : '#1e293b') : '#6b7280')
        .text(d.region);
  ...
  function initCountrySvg(region) {
    ...
    if (hdr) hdr.innerHTML = `<span style="color:${isCEE?ORANGE_MID:BLUE_STRONG};">${region}</span> 地区内各国${currentPeriod === 'p16' && currentMetric === 'growth' ? '增幅排名' : '占比排名'}`;
  ...
  function updateCountrySvg(region, animate) {
    ...
    const barColor = d => d.cee ? ORANGE_MID : (REGION_COLOR[d.region] || '#6b7280');
    ...
    if (titleNode) titleNode.innerHTML = `<span style="color:${isCEERegion?ORANGE_MID:BLUE_STRONG};">${region}</span> 地区内各国${showGrowth ? '增幅排名' : '占比排名'}`;
  ...
  function updateCallout() {
    ...
    if (!el) return;
    el.innerHTML = `
      <div style="background:#ffffff;border-radius:10px;padding:16px 18px;color:#111827;">
        <div style="display:flex;flex-direction:column;gap:13px;">
          <div style="font-size:15px;font-weight:800;line-height:1.5;color:#111827;">为什么聚焦中东欧：</div>

          <div style="display:flex;flex-direction:column;gap:5px;">
            <div style="font-size:13px;font-weight:800;line-height:1.55;color:#111827;">1. 合作学科总体扩展明显</div>
            <div style="color:#111827;font-size:12.5px;line-height:1.65;padding-left:16px;">
              2016–2020 年论文总量 <strong style="color:#b45309;">39,172 篇</strong>，相比 2011–2015 年的 <strong style="color:#b45309;">17,761 篇</strong>增长 <strong style="color:#16a34a;">+120.5%</strong>
            </div>
            <div style="color:#111827;font-size:12.5px;line-height:1.65;padding-left:16px;">
              理学、工学和医学仍为合作核心，占比约 <strong style="color:#b45309;">39.1%</strong>、<strong style="color:#b45309;">39.1%</strong>、<strong style="color:#b45309;">12.0%</strong>，学科分布稳定支撑核心合作方向
            </div>
          </div>

          <div style="display:flex;flex-direction:column;gap:5px;">
            <div style="font-size:13px;font-weight:800;line-height:1.55;color:#111827;">2. 学科多样化趋势增强</div>
            <div style="color:#111827;font-size:12.5px;line-height:1.65;padding-left:16px;">
              “其他”类别从 2011–2015 的占比 <strong style="color:#b45309;">5.2%</strong> 上升，说明合作逐渐覆盖更多学科领域
            </div>
            <div style="color:#111827;font-size:12.5px;line-height:1.65;padding-left:16px;">
              支撑中东欧各国在合作中形成差异化研究方向
            </div>
          </div>

          <div style="display:flex;flex-direction:column;gap:5px;">
            <div style="font-size:13px;font-weight:800;line-height:1.55;color:#111827;">3. 政策上有牵引</div>
            <div style="color:#111827;font-size:12.5px;line-height:1.65;padding-left:16px;">
              <strong style="color:#b45309;">2012 年</strong>中国—中东欧合作机制建立，为区域合作提供制度化背景，也使中东欧成为中国对外合作中具有明确政策指向的区域。
            </div>
          </div>
        </div>
      </div>
  ...
  function switchPeriod(p) {
    ...
    if (p === 'p11') {
      b11.style.background = BLUE_STRONG; b11.style.color = '#fff';
      b16.style.background = '#f8fafc'; b16.style.color = '#64748b';
    } else {
      b16.style.background = BLUE_STRONG; b16.style.color = '#fff';
