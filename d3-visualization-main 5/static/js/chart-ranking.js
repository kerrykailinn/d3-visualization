...
(function () {
  ...
  const BLUE_STRONG = '#1f3f6f';
  const BLUE_MID = '#587fae';
  const BLUE_LIGHT = '#c7d8ea';
  ...
  function initTooltip() {
    ...
    window.cee4ShowTip = function(e, d) {
      ...
      const grNum = gr === '—' ? null : +gr;
      const grColor = grNum == null ? '#94a3b8' : grNum >= 0 ? POSITIVE : NEGATIVE;
      ...
      tt.innerHTML = `
        ...
        <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
          ...
          <span style="color:${BLUE_LIGHT};font-weight:600;">${fmt(d.p16)} 篇</span>
        </div>
        <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:3px;">
          <span style="color:rgba(255,255,255,.5);">增幅</span>
          <span style="color:${grColor};font-weight:600;">${gr === '—' ? '—' : `${grNum >= 0 ? '+' : ''}${gr}%`}</span>
        </div>
        <div style="display:flex;justify-content:space-between;gap:12px;">
          <span style="color:rgba(255,255,255,.5);">区域占比</span>
          <span style="color:${ORANGE_MID};">${s11}% → ${s16}%</span>
        </div>`;
      ...
      <div style="background:#fff;border-radius:14px;padding:22px 24px;
        ...
        <div style="margin-bottom:18px;">
          ...
          <div style="font-size:11.5px;color:#64748b;">
            悬停地图或排名图 → 双向联动高亮 ·
                <span style="color:${ORANGE_MID};font-weight:600;">橙色</span>深浅 = 2016–2020发文量 ·
            增速在悬停时显示
          </div>
        </div>
  ...
  function redrawMapAtSize(W, H) {
    ...
    svg.append('g').attr('class','cee4-map-cee')
      ...
      .attr('fill', f=>{ const d=ceeByNum[f.id]; return d?colorScale(d.p16):'#e8eef3'; })
        .attr('stroke',ORANGE_STRONG).attr('stroke-width',1.2).attr('cursor','pointer')
  ...
  function renderRankBars() {
    ...
    data.forEach((d,i)=>{
      ...
      g.append('rect')
        ...
        .attr('height',BAR).attr('width',0)
        .attr('fill',ORANGE_MID).attr('fill-opacity',1)
        .transition().duration(500).delay(i*22)
        .attr('width', x(d.p16));
      ...
      g.append('text')
        ...
        .attr('font-size',9).attr('fill',ORANGE_STRONG)
  ...
  function applyHighlight(cn) {
    ...
    d3.select('#cee4-map-svg').select('g.cee4-map-cee').selectAll('path')
      ...
      .attr('stroke', f=> ceeByNum[f.id]?.cn===cn ? '#0f172a' : ORANGE_STRONG)
  ...
  function clearHighlight() {
    ...
    d3.select('#cee4-map-svg').select('g.cee4-map-cee').selectAll('path')
      .attr('fill-opacity',1)
      .attr('stroke',ORANGE_STRONG).attr('stroke-width',1.2);
  }
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        ...
        <tbody>
          ${REGION_GROWTH.map((d,i)=>{
            ...
            const grColor = d.growth >= 0 ? POSITIVE : NEGATIVE;
            const bg = d.isCEE? '#fff7ed' : i%2===0?'#fafbfc':'#fff';
            return `
              <tr style="border-bottom:1px solid #f1f5f9;background:${bg};">
                <td style="padding:8px 10px;font-weight:${d.isCEE?'700':'400'};
                  color:${d.isCEE?ORANGE_STRONG:'#374151'};white-space:nowrap;">
                  ${d.isCEE?'★ ':''}${d.r}
                </td>
                ...
                <td style="padding:8px 10px;text-align:right;font-weight:${d.isCEE?'700':'500'};
                  color:${d.isCEE?ORANGE_STRONG:'#374151'};">
                  ...
                  <div style="display:flex;align-items:center;gap:8px;">
                    <div style="flex:1;height:10px;background:#f1f5f9;border-radius:3px;overflow:hidden;">
                      <div style="height:100%;width:${barW}%;
                        background:${d.isCEE?ORANGE_MID:grColor};border-radius:3px;"></div>
                    </div>
                    <span style="font-weight:700;font-size:11px;min-width:52px;text-align:right;
                      color:${grColor};">${d.growth >= 0 ? '+' : ''}${d.growth}%</span>
                  </div>
                </td>
              </tr>`;
          }).join('')}
          <tr style="border-top:2px solid #e2e8f0;background:#f0f7ff;">
            <td style="padding:8px 10px;font-weight:700;color:${BLUE_STRONG};">全球平均</td>
            <td style="padding:8px 10px;text-align:right;color:#94a3b8;">${d3.format('.0s')(GLOBAL.p11)}</td>
            <td style="padding:8px 10px;text-align:right;font-weight:700;color:${BLUE_STRONG};">${d3.format('.0s')(GLOBAL.p16)}</td>
            <td style="padding:8px 14px;">
              <div style="display:flex;align-items:center;gap:8px;">
                <div style="flex:1;height:10px;background:#f1f5f9;border-radius:3px;overflow:hidden;">
                  <div style="height:100%;width:${(GLOBAL.growth/maxGrowth*100).toFixed(1)}%;
                    background:${POSITIVE};border-radius:3px;"></div>
                </div>
                <span style="font-weight:700;font-size:11px;min-width:52px;text-align:right;color:${POSITIVE};">+${GLOBAL.growth}%</span>
              </div>
            </td>
          </tr>
        </tbody>
