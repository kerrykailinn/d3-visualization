...
(function () {
  ...
  const CATEGORY_COLORS = [
    '#A0D8E6', '#79B2C9', '#5A8DAB', '#003366', '#1F5360','#6D9DC5'
  ];
  ...
  function init() {
    ...
    buildDOM(root);
    Promise.all([
      window.__loadLocalCsvRows ? window.__loadLocalCsvRows([PATH_CATEGORY, `./${PATH_CATEGORY}`]) : d3.csv(PATH_CATEGORY),
      window.__loadLocalCsvRows ? window.__loadLocalCsvRows([PATH_SUB, `./${PATH_SUB}`]) : d3.csv(PATH_SUB),
      window.__loadLocalCsvRows ? window.__loadLocalCsvRows([PATH_GLOBAL, `./${PATH_GLOBAL}`]) : d3.csv(PATH_GLOBAL)
  ...
  function buildDOM(root) {
    ...
    left.append('h3').attr('class', 'disc-title').text('一、学科结构对比（2011–2015 vs 2016–2020）');
  ...
  function drawPie() {
    ...
    // helper: compute approximate text block height (lineCount * lineHeight)
    function lineBlockH(lineCount) { return (lineCount + 0.3) * 1.15 + 'em'; }
    ...
    g.append('text').attr('text-anchor', 'middle').attr('dominant-baseline', 'middle')
      ...
      .attr('y', 0)
      .selectAll('tspan').data(['内圈 2011–2015', '外圈 2016–2020'])
      .enter().append('tspan').attr('x', 0).attr('dy', (d, i) => i === 0 ? '-1.1em' : '1.3em')
      .text(d => d);
  }
  ...
  function drawBarsForCategory(category) {
    ...
    const chartTitleMap = {
      '全部学科': { label: '全部学科 — 细分学科堆叠', topLabel: '全部学科 — 细分学科堆叠', bottomLabel: '' },
    };
    const catKey = category || '全部学科';
    const catInfo = chartTitleMap[catKey] || { label: `学科大类：${category} — 细分学科分布`, topLabel: '', bottomLabel: '' };
    titleBar.append('div').style('font-size', '13px').style('color', '#0f172a').style('font-weight', '600')
      .text(catInfo.label || `学科大类：${category} — 细分学科分布`);
    ...
    // 2011 row: flex so each segment is proportionally equal-width
    const row1Wrap = g.append('g').attr('transform', 'translate(0,0)');
    const row1FO = row1Wrap.append('foreignObject')
      .attr('width', svgW).attr('height', rowH - 2).attr('x', 0).attr('y', 0);
    const row1Div = row1FO.append('xhtml:div')
      .style('display', 'flex').style('height', `${rowH - 2}px`);
    displaySubs.forEach(d => {
      const pct = total11 ? (d.p11 / total11 * 100) : 0;
      const seg = row1Div.append('xhtml:div')
        .style('background', d._color)
        .style('flex', `${pct.toFixed(2)} 1 0%`)
        .style('height', '100%').style('box-sizing', 'border-box')
        .style('border-right', '1px solid #fff')
        .on('mouseenter', function(e) { e.stopPropagation(); showSubTooltip(e, d, '2011–2015', total11); })
        .on('mousemove', moveSubTooltip)
        .on('mouseleave', hideSubTooltip);
      seg.append('xhtml:span')
        .style('display', 'flex').style('align-items', 'center').style('justify-content', 'center')
        .style('width', '100%').style('height', '100%').style('box-sizing', 'border-box')
        .style('font-size', '10px').style('font-weight', '600').style('color', '#fff')
        .style('text-align', 'center').style('padding', '0 3px')
        .text(pct >= 4 ? `${pct.toFixed(1)}%` : '');
    });

    // 2016 row
    const row2Wrap = g.append('g').attr('transform', `translate(0,${rowH + rowGap})`);
    const row2FO = row2Wrap.append('foreignObject')
      .attr('width', svgW).attr('height', rowH - 2).attr('x', 0).attr('y', 0);
    const row2Div = row2FO.append('xhtml:div')
      .style('display', 'flex').style('height', `${rowH - 2}px`);
    displaySubs.forEach(d => {
      const pct = total16 ? (d.p16 / total16 * 100) : 0;
      const seg = row2Div.append('xhtml:div')
        .style('background', d._color)
        .style('flex', `${pct.toFixed(2)} 1 0%`)
        .style('height', '100%').style('box-sizing', 'border-box')
        .style('border-right', '1px solid #fff')
        .on('mouseenter', function(e) { e.stopPropagation(); showSubTooltip(e, d, '2016–2020', total16); })
        .on('mousemove', moveSubTooltip)
        .on('mouseleave', hideSubTooltip);
      seg.append('xhtml:span')
        .style('display', 'flex').style('align-items', 'center').style('justify-content', 'center')
  ...
  function renderGlobalPanel(el) {
    ...
    // Insight card
    el.append('div').attr('class', 'dp-insight-card dp-core-finding-card')
    ...
    addStatCard(grid, '合作论文总数（2011-2015）', (+g.total_2011_all).toLocaleString(), '篇论文', false);
    addStatCard(grid, '合作论文总数（2016-2020）', (+g.total_2016_all).toLocaleString(), '篇论文', false);
    addStatCard(grid, '论文总数增长量', (+g.total_increase).toLocaleString(), '篇', false);
    addStatCard(grid, '论文总数增长率', fmtGrowth(g.total_growth_rate), '', true);
    ...
    state.categoryRankings.forEach((cat, i) => {
      ...
      row.append('div').attr('class', 'dp-ranking-stat').style('color', '#16a34a').text(fmtGrowth(cat.category_growth_rate));
    });
  ...
  function renderCategoryPanel(el, cat) {
    ...
    // Insight
    const categoryGrowthText = fmtGrowth(catData.category_growth_rate);
    const categoryGrowthColor = categoryGrowthText.startsWith('-') ? '#dc2626' : '#16a34a';
    el.append('div').attr('class', 'dp-insight-card dp-discipline-summary-card').style('margin-bottom', '14px')
      .html(`<div class="dp-insight-title">学科概况</div>
        <div class="dp-insight-body">
          ...
          较上一周期增长 <strong style="color:${categoryGrowthColor};">${categoryGrowthText}</strong>，
    ...
    // Stats
    const grid = el.append('div').attr('class', 'dp-stat-grid');
    addStatCard(grid, '合作论文总数(2011-2015)', catData.category_2011_total.toLocaleString(),
      `${(catData.category_2011_ratio * 100).toFixed(1)}% 占比`, false);
    addStatCard(grid, '合作论文总数(2016-2020)', catData.category_2016_total.toLocaleString(),
      `${(catData.category_2016_ratio * 100).toFixed(1)}% 占比`, false);
    addStatCard(grid, '论文总数增长量', `+${catData.category_increase.toLocaleString()}`, '篇', false);
    addStatCard(grid, '论文总数增长率', fmtGrowth(catData.category_growth_rate), '', true);
    ...
    catSubs.forEach((sub, i) => {
      ...
      row.append('div').attr('class', 'dp-ranking-stat').style('color', '#16a34a').text(fmtGrowth(sub.sub_growth_rate));
    });
  ...
  function addStatCard(grid, label, value, sub, isGrowth) {
    let cls = 'dp-stat-card';
    if (isGrowth) {
      const num = parseFloat(String(value).replace('%', ''));
      cls += Number.isNaN(num) ? ' dp-growth' : (num >= 0 ? ' dp-growth' : ' dp-decline');
    }
    const card = grid.append('div').attr('class', cls);
  ...
  function showSubTooltip(event, d, period, total) {
    ...
    const growthNum = parseFloat(String(growth).replace('%', ''));
    const growthColor = Number.isNaN(growthNum) ? '#ffffff' : (growthNum >= 0 ? '#86efac' : '#fca5a5');
    tip.style('display', 'block').html(
      `<div style="font-weight:600;margin-bottom:6px">${d.discipline_cn}</div>
       ...
       <div>增长率：<span style="color:${growthColor};font-weight:600;">${growth}</span></div>`
    );
  ...
  function injectStyles() {
    const css = `
      ...
      .disc-title { margin: 0 0 6px 0; font-weight: 700; font-size: 1.5rem; line-height: 1.25; color: #0f172a; }
      .disc-subtitle { color: #6b7280; font-size: 1rem; line-height: 1.7; }
