...
(function () {
  ...
  const IL_SECTIONS = {
    macro: {
      ...
      label: "分布结构",
      title: "整体集中度",
      sub: "合作是否仍集中于少数核心机构？中国与中东欧两侧的合作结构，是否正在从集中走向扩散？<br><span class=\"il-section-sub-note\">注：横轴为前期份额、纵轴为后期份额；气泡大小表示份额变化幅度，颜色区分中国/中东欧；对角线上方为增长、下方为下降，主图聚焦高份额及变化 Top15 机构。</span>"
    },
    sankey: {
      figure: "图 07",
      ordinal: "二",
    ...
    bubble: {
      ...
      label: "份额结构",
      title: "机构结构份额迁移",
      sub: "哪些机构是稳定头部？哪些是新兴增长机构？哪些在退场？以两期合作份额对照观察。<br><span class=\"il-section-sub-note\">注：横轴为前期份额、纵轴为后期份额；气泡大小表示份额变化幅度，颜色区分中国/中东欧。</span>"
    },
    meso: {
      figure: "图 09",
      ordinal: "四",
  ...
  function buildMesoOverviewSideHtml(region) {
    ...
    const color = region === "china" ? "rgb(37, 99, 235)" : "#d97706";
    return (
      `<div class="meso-overview-stats">` +
      bubbleSideStatCard("正增速", st.up, "家", color) +
      bubbleSideStatCard("负增速", st.down, "家", color) +
      bubbleSideStatCard("新入 Top15", st.newcomer, "家", color) +
      bubbleSideStatCard("持续 Top15", st.incumbent, "家", color) +
      `</div>`
    );
  }
  ...
  function renderMacroStatCardRow(grid, cards, opts = {}) {
    ...
    innerEnter.append("span").attr("class", "macro-stat-delta");
  ...
  function renderMacroStatCards(root, opts = {}) {
    ...
    root.selectAll(".macro-stat-insight").remove();
    ...
    if (S.macroMode === "region") {
      ...
      root
        .append("div")
        .attr("class", "macro-stat-insight")
        .text("中东欧整体集中度仍高于中国，但去集中化幅度更明显。");
      return;
    }
    ...
    root
      ...
      .text(
        S.macroRegion === "china"
          ? "中国 Top5 / Top10 / Top20 分额均下降，说明合作不再过度依赖少数核心机构。"
          : "中东欧整体集中度仍高于中国，但去集中化幅度更明显。"
      );
  }
  ...
  function bubbleRegionStatPalette(region) {
    if (region === "china") {
      return {
        head: C.china,
        growth: "rgb(37, 99, 235)",
        stable: "rgb(37, 99, 235)",
        decline: "rgb(37, 99, 235)",
        emerging: "rgb(37, 99, 235)",
  ...
  function init() {
    ...
    Promise.all([
      window.__loadLocalCsvRows ? window.__loadLocalCsvRows(["static/data/china_institution.csv", "./static/data/china_institution.csv"]) : d3.csv("static/data/china_institution.csv"),
      window.__loadLocalCsvRows ? window.__loadLocalCsvRows(["static/data/cee_institution.csv", "./static/data/cee_institution.csv"]) : d3.csv("static/data/cee_institution.csv"),
      window.__loadLocalCsvRows ? window.__loadLocalCsvRows(["country.csv", "./country.csv"]) : d3.csv("country.csv")
    ])
