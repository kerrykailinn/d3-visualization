(function () {
  ...
  function createDOM() {
    // inner title is suppressed by CSS, but we keep for structure
    STATE.container.append('div').attr('class', 'country-treemap-title')
      .style('display', 'none');
  ...
  function renderLabels(countries) {
    ...
    countries.forEach(country => {
      ...
      const size = Math.max(12, Math.min(20, Math.round(Math.min(STATE.dimensions.width, STATE.dimensions.height) / 40)));
  ...
  function renderDashboardDefault() {
    ...
    const titleGroup = header.append('div').attr('class', 'ct-panel-title-group');
    titleGroup.append('div').attr('class', 'ct-panel-country-name').text('Top10 国家概览');
    titleGroup.append('div').attr('class', 'ct-panel-rank').text('中东欧主要国家');
    ...
    addStat('全球占比', pct.toFixed(1) + '%', 'Top10 / 全球');
